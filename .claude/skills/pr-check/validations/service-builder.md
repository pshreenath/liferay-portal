...
Autogenerated outputs are **not** triggers on their own — drift is caught by `git diff --quiet`. An autogenerated `*ModelImpl.java` hand-edited without a corresponding input change is an antipattern (the next regen will overwrite it); flag the change but do not fire this validation.
